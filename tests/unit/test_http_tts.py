...
from features.tts.http_tts import HttpTTSClient, create_omnivoice_client
...
class TestHttpTTS:
    ...
    def test_generate_speech_raises_on_empty_response_body(self, tmp_path):
        with patch.dict(os.environ, {"OMNIVOICE_API_URL": "http://localhost:5003"}):
            client = create_omnivoice_client()
            output_path = str(tmp_path / "out.wav")
    ...
    def test_generate_speech_cleans_up_on_write_failure(self, tmp_path):
        client = HttpTTSClient("http://localhost:5003", 30, "Piper")
        output_path = str(tmp_path / "out.wav")
