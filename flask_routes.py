...
def create_flask_app(
    ...
    def _run_tts_speak(timeout: int, provider: str | None = None):
    ...
    @require_bot_ready
    def tts_speak():
        timeout = int(os.getenv('OMNIVOICE_TIMEOUT', '90')) if os.getenv('TTS_PROVIDER', 'elevenlabs') == 'omnivoice' else 30
        return _run_tts_speak(timeout)

    @flask_app.route('/tts/piper/speak', methods=['POST'])
    @require_bot_ready
    def tts_piper_speak():
        return _run_tts_speak(30, 'piper')
    ...
    def tts_omnivoice_speak():
        ...
        return _run_tts_speak(int(os.getenv('OMNIVOICE_TIMEOUT', '90')), 'omnivoice')
