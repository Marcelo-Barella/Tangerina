...
SMOKE_PHRASE="${SMOKE_PHRASE:-Olá, este é um teste de voz do Tangerina.}"
...
run_sidecar_roundtrip() {
  ...
  curl -fsS -X POST "${tts_url}/tts" \
    ...
    -d "$(python3 -c 'import json, os; print(json.dumps({"text": os.environ["SMOKE_PHRASE"]}))')" \
    -o "$wav_path"
  ...
  transcript="$(curl -fsS -X POST "${WHISPER_URL}/transcribe" \
    -F "file=@${wav_path};type=audio/wav" | python3 -c 'import json,sys; print(json.load(sys.stdin).get("text","").strip())')"
...
run_bot_roundtrip() {
  ...
  curl -fsS -X POST "${BOT_URL}/tts/preview" \
    -H 'Content-Type: application/json' \
    -d "$(python3 -c 'import json, os; print(json.dumps({"text": os.environ["SMOKE_PHRASE"], "provider": "piper"}))')" \
    -o "$wav_path"
