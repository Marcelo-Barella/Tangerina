...
class VoiceCommandSink(BaseSink):
    ...
    if voice_recv:
        ...
        def on_voice_member_speaking_stop(self, member):
            if member.id in self.speaking_users:
                ...
                if hasattr(self, 'music_bot_ref'):
                    music_bot = self.music_bot_ref
                    loop = music_bot.main_loop or (self.bot.loop if hasattr(self.bot, 'loop') and self.bot.loop else None)
    ...
    if voice_recv:
        @voice_recv.AudioSink.listener()
        def on_voice_member_speaking_start(self, member):
            self.on_voice_member_speaking_start(member)
        
        @voice_recv.AudioSink.listener()
        def on_voice_member_speaking_stop_listener(self, member):
            if hasattr(self, 'music_bot_ref'):
                self.on_voice_member_speaking_stop(member, self.music_bot_ref)
        ...
        try:
            ...
            for i in range(0, len(stereo_samples), 2):
                if i + 1 < len(stereo_samples):
                    mono_value = (stereo_samples[i] + stereo_samples[i + 1]) // 2
    ...
    async def _handle_chat(self, channel, text):
        ...
        try:
            response = await self.chatbot.generate_response(chat_text)
